...
class InstructorViewModel(
    ...
    tokenStore: InstructorTokenStore,
) : ViewModel() {
    ...
    fun createCourse(requestId: String?, title: String, description: String) {
        viewModelScope.launch {
            _state.update { it.copy(isLoading = true, message = null) }
            repository.createCourse(requestId, title, description)
                .onSuccess {
                    _state.update { it.copy(message = "Curso criado como entrega/rascunho.") }
                    refreshAll()
                }
                .onFailure { err -> _state.update { it.copy(isLoading = false, message = err.message) } }
        }
