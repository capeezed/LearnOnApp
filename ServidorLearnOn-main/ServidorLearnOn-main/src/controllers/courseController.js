    ...
    if (studentIds.length) {
      await conn.query(
        ...
        [studentIds.map((studentId) => [
          ...
          'Seu microcurso ficou pronto',
          `O curso "${course.title}" ja esta pronto. Pague para liberar o acesso.`,
        ])]
      );
    }
    ...
    `SELECT *
     FROM (
       SELECT c.id, c.title, c.format, c.duration_minutes, c.thumbnail_url,
              c.price, cr.topic_tag, c.status,
              CASE
                WHEN COALESCE(c.price, 0) > 0 AND approved_payment.id IS NULL THEN 0
                ELSE COALESCE(p.percent_complete, 0)
              END AS progress,
              p.last_position_sec,
              e.enrolled_at,
              CASE
                WHEN COALESCE(c.price, 0) > 0 AND approved_payment.id IS NULL THEN 'payment_required'
                ELSE 'unlocked'
              END AS access_status,
              CASE
                WHEN latest_payment.status IN ('paid', 'credited') THEN 'approved'
                ELSE latest_payment.status
              END AS payment_status,
              latest_payment.id AS payment_id,
              NULL AS checkout_url,
              NULL AS sandbox_checkout_url,
              c.published_at
       FROM enrollments e
       JOIN courses c ON c.id = e.course_id
       LEFT JOIN course_requests cr ON cr.id = c.origin_request_id
       LEFT JOIN progress p ON p.enrollment_id = e.id
       LEFT JOIN (
         SELECT p1.*
         FROM payments p1
         JOIN (
           SELECT student_id, course_id, MAX(id) AS id
           FROM payments
           GROUP BY student_id, course_id
         ) lp ON lp.id = p1.id
       ) latest_payment ON latest_payment.student_id = e.student_id AND latest_payment.course_id = c.id
       LEFT JOIN payments approved_payment
         ON approved_payment.student_id = e.student_id
        AND approved_payment.course_id = c.id
        AND approved_payment.status IN ('paid', 'credited')
       WHERE e.student_id = ?

       UNION ALL

       SELECT DISTINCT c.id, c.title, c.format, c.duration_minutes, c.thumbnail_url,
              c.price, cr.topic_tag, c.status,
              0 AS progress,
              NULL AS last_position_sec,
              NULL AS enrolled_at,
              CASE
                WHEN latest_payment.status IN ('paid', 'credited') THEN 'processing'
              ...
              c.published_at
       FROM courses c
       JOIN course_requests cr ON (
         cr.id = c.origin_request_id
         OR EXISTS (
           SELECT 1
           FROM request_group_members rgm
           WHERE rgm.group_id = c.request_group_id AND rgm.request_id = cr.id
         )
       )
       LEFT JOIN (
         SELECT p1.*
       ...
       ) latest_payment ON latest_payment.student_id = cr.student_id AND latest_payment.course_id = c.id
       WHERE cr.student_id = ?
         AND c.status = 'published'
         AND NOT EXISTS (
           SELECT 1
           FROM enrollments e
           WHERE e.student_id = cr.student_id AND e.course_id = c.id
         )
     ) mine
     ORDER BY COALESCE(enrolled_at, published_at) DESC`,
...
const saveProgress = asyncHandler(async (req, res) => {
  ...
  const [[enrollment]] = await db.query(
    `SELECT e.id, c.price, approved_payment.id AS approved_payment_id
     FROM enrollments e
     JOIN courses c ON c.id = e.course_id
     LEFT JOIN payments approved_payment
       ON approved_payment.student_id = e.student_id
      AND approved_payment.course_id = e.course_id
      AND approved_payment.status IN ('paid', 'credited')
  ...
  if (Number(enrollment.price || 0) > 0 && !enrollment.approved_payment_id) {
    throw new AppError('Pagamento necessario para acessar este curso.', 402);
  }
