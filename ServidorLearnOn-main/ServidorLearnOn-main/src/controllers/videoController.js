...
async function resolveStudentEnrollment(courseId, studentId) {
  const [[enrollment]] = await db.query(
    `SELECT e.id, c.status, c.price,
            approved_payment.id AS approved_payment_id
     FROM enrollments e
...
async function canReadVideo(videoId, user) {
  const [[video]] = await db.query(
    'SELECT * FROM course_videos WHERE id = ? LIMIT 1',
    [videoId]
  );
...
const createVideo = asyncHandler(async (req, res) => {
  ...
  if (!videoUrl) {
    throw new AppError('Envie um arquivo de video ou informe video_url.', 422);
  }

  const thumbnailUrl = req.body.thumbnail_url
    || upload?.eager?.[0]?.secure_url
    || (upload?.public_id
      ? cloudinary.url(upload.public_id, { resource_type: 'video', format: 'jpg', width: 640, crop: 'scale' })
      : null);
...
const getVideo = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== 'student') {
    return res.json(videoToApi(video));
  }
...
const saveVideoProgress = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== 'student') {
    throw new AppError('Apenas alunos salvam progresso de video.', 403);
  }
  ...
  await db.query(
    ...
    [
      ...
      percent.toFixed(2),
      lastPositionSec,
      percent >= 90 ? new Date() : null,
    ]
  );

  res.json({
    percent_complete: percent.toFixed(2),
