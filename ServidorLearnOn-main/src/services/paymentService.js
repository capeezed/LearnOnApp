...
const FINAL_STATUSES = new Set(['approved', 'rejected', 'cancelled', 'refunded', 'charged_back', 'expired']);
...
async function createCheckout(student, courseId) {
  ...
  try {
    ...
    const [[course]] = await conn.query(
      `SELECT c.id, c.title, c.description, c.price, c.status, c.thumbnail_url,
    ...
    if (!course || course.status !== 'published') throw new AppError('Curso nao encontrado ou indisponivel.', 404);

    const [[enrollment]] = await conn.query(
      'SELECT id FROM enrollments WHERE student_id = ? AND course_id = ? LIMIT 1',
      [student.id, course.id]
    );
    if (enrollment) throw new AppError('Voce ja possui acesso a este curso.', 409);

    const [[pending]] = await conn.query(
      `SELECT * FROM payments
       WHERE student_id = ? AND course_id = ? AND status IN ('pending', 'in_process')
    ...
    const [result] = await conn.query(
      `INSERT INTO payments
         (student_id, course_id, provider, status, amount, currency, external_reference)
       VALUES (?, ?, 'mercado_pago', 'pending', ?, 'BRL', ?)`,
      [student.id, course.id, amount.toFixed(2), reference]
    );
    ...
    await conn.query(
      `UPDATE payments
       SET provider_preference_id = ?, checkout_url = ?, sandbox_checkout_url = ?, raw_provider_response = ?
       WHERE id = ?`,
      [
        preference.id,
...
async function syncApprovedPayment(providerPaymentId, req = null) {
  ...
  try {
    ...
    const [[payment]] = await conn.query(
      'SELECT * FROM payments WHERE external_reference = ? FOR UPDATE',
      [externalReference]
    );
    ...
    await conn.query(
      `INSERT INTO payment_events
         (payment_id, provider, provider_event_id, provider_payment_id, event_type, payload)
       VALUES (?, 'mercado_pago', ?, ?, ?, ?)`,
      [
        payment.id,
        providerPayment.id,
    ...
    if (providerAmount + 0.001 < expectedAmount) {
    ...
    const nextStatus = providerStatus === 'approved'
      ? 'approved'
      : (FINAL_STATUSES.has(providerStatus) ? providerStatus : 'in_process');

    await conn.query(
      `UPDATE payments
       SET status = ?, provider_payment_id = ?, payment_method = ?, payment_type = ?,
           paid_at = COALESCE(paid_at, ?), raw_provider_response = ?
       WHERE id = ?`,
      [
        nextStatus,
        providerPayment.id,
        providerPayment.payment_method_id || null,
        providerPayment.payment_type_id || null,
        paidAt,
        JSON.stringify(providerPayment),
        payment.id,
      ]
